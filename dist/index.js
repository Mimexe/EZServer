...
async function confirm(message = "Are you sure?") {
    const { confirm } = await inquirer.prompt([
        {
            type: "confirm",
            name: "confirm",
...
async function createServer(type, version, name, port) {
    logger.info("Creating %s %s server on folder %s on 127.0.0.1:%s", type, version, name, port);
    ...
    fs.writeFileSync(`./${server.name}/start.bat`, `${getCommand(server.type)}\npause`);
    fs.writeFileSync(`./${server.name}/server.properties`, `server-port=${port}\n`);
    ...
    logger.info("Start 'start.bat' or '" + getCommand(server.type) + "' to start the server");
    process.exit(0);
}
async function main() {
    try {
        ...
        logger.error("Fatal error while running program:\n" + error.stack ||
            error.message ||
            error);
        process.exit(1);
    }
